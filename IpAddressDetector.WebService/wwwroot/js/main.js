        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.err) {
                    ...
                    if (resp.ips && resp.ips.length) {
                        var htmls = [], $div = $('<div>'), startIndex = 0;
                        for (var i = 0, len = resp.ips.length; i < len; i++) {
                            var ip = resp.ips[ i ];
                            //text = text.insert( ip.startIndex + ip.length, '</span>' ).insert( ip.startIndex, '<span class="ip">' );
                            var ip_value    = $div.text( text.substr(ip.startIndex, ip.length) ).html(),
                                before_text = $div.text( text.substr(startIndex, ip.startIndex - startIndex) ).html();
                            htmls.push( before_text + '<span class="ip" title=' + ip_value + '>' + ip_value + '</span>' );
                        ...
                        $('#resultCount').text('found IP (v4) addresses: ' + resp.ips.length);
                    } else {
                        processing_end();
